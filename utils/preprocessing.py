...
# TODO: Add more logging per augmentation
...
def apply_flipping(image, label, aff, left_right_corresponding, flip_prob=0.5):
    ...
    if np.random.rand() < flip_prob:
        ...
        assert (
            left_right_corresponding is not None
        ), "left_right_corresponding should not be None when applying flipping"
        ...
        right_labels = left_right_corresponding[
            np.arange(start=1, stop=n_left_right_labels, step=2)
        ]
...
def apply_randomcrop(image, label, crop_size, mode="random", bbox_labels=None, debug=False):
    ...
    if mode == "random":
        ...
        end_coords = np.minimum(np.array(center_point) + half_crop, np.array(image_shape)).astype(
            int
        )
...
def apply_sampleConditionalGMM(
    label_map, expected_classes, prior_mean=[25, 225], prior_std=[5, 25], distribution="uniform"
):
    """
    ...
    for labelid in range(n_classes):
        ...
        gauss_samples = np.random.normal(
            loc=means[:, labelid], scale=stds[:, labelid], size=(batchsize, indices_count)
        )
...
def apply_biasFieldCorruption(image, bias_field_std=0.5, bias_scale=0.025, prob=0.95):
    """
    The input tensors are expected to have shape [batch H W D].
    ...
    small_bias_shape = [batchsize] + [
        math.ceil(image_shape[i] * bias_scale) for i in range(len(image_shape))
    ]
    ...
    bias_field_tensor = torch.nn.functional.interpolate(
        bias_field_tensor.unsqueeze(1), image_shape, mode="trilinear"
    )
...
def apply_intensityAugmentation(
    image,
    noise_std=0,
    clip_values=[0, 300],
    normalise=True,
    gamma_std=0,
    contrast_inversion=False,
    prob_noise=0.95,
    prob_gamma=1,
):
    """
...
def apply_augmentations(
    ...
    if augmentations_to_apply is None:
        augmentations_to_apply = cfg.preprocessing.train_augmentations
    ...
    for aug in augmentations_to_apply:
        if aug == 'flipping':
            image_tensor, label_tensor = apply_flipping(
                image_tensor, 
                label_tensor, 
                original_image.geom.vox2world.matrix,
                cfg.preprocessing.left_right_corresponding,
                cfg.preprocessing.flip_prob
            )
        ...
        if aug == 'spatial_transform':
            image_tensor, label_tensor = apply_spatial_transform(
                image_tensor, 
                label_tensor, 
                original_image.geom.voxsize,
                affine_probability=cfg.preprocessing.affine_probability,
                max_translation=cfg.preprocessing.max_translation,
                max_rotation=cfg.preprocessing.max_rotation,
                max_scaling=cfg.preprocessing.max_scaling,
                warp_probability=cfg.preprocessing.warp_probability,
                warp_integrations=cfg.preprocessing.warp_integrations,
                warp_smoothing_range=cfg.preprocessing.warp_smoothing_range,
                warp_magnitude_range=cfg.preprocessing.warp_magnitude_range,
                shearing_bounds=cfg.preprocessing.shearing_bounds,
            )
            if save_volumes is not None and output_dir is not None:
                save_volume(
                    image_tensor,
                    original_image,
                    os.path.join(output_dir, save_volumes + "spatially_transformed_image.mgz"),
                )
                save_volume(
                    label_tensor,
                    original_label,
                    os.path.join(output_dir, save_volumes + "spatially_transformed_label.mgz"),
                )

        if aug == 'cropping':
            # ??? we are now supporting cropping, randomcrop, randomcrop_center. check to allow only one type of cropping ???
            image_tensor, label_tensor = apply_cropping(image_tensor, label_tensor, cfg.preprocessing.crop_size)
            if save_volumes is not None and output_dir is not None:
                save_volume(
                    image_tensor,
                    original_image,
        ...
        if aug == 'randomcrop':
            image_tensor, label_tensor = apply_randomcrop(
                image_tensor,
                label_tensor,
                cfg.preprocessing.crop_size,
                mode="random",
                bbox_labels=cfg.preprocessing.bbox_labels,
                debug=True if cfg.preprocessing.debug else False,
            )
            if save_volumes is not None and output_dir is not None:
                save_volume(
                    image_tensor,
        ...
        if aug == 'blur_resample':
            image_tensor = apply_blur_resample(
                image_tensor,
                original_image.geom.voxsize,
                smoothing_probability=cfg.preprocessing.smoothing_probability,
                smoothing_max_sigma=cfg.preprocessing.smoothing_max_sigma,
                added_noise_probability=cfg.preprocessing.added_noise_probability,
                added_noise_max_sigma=cfg.preprocessing.added_noise_max_sigma,
                gamma_scaling_probability=cfg.preprocessing.gamma_scaling_probability,
                gamma_scaling_max=cfg.preprocessing.gamma_scaling_max,
                resized_probability=cfg.preprocessing.resized_probability,
                resized_one_axis_probability=cfg.preprocessing.resized_one_axis_probability,
                resized_max_voxsize=cfg.preprocessing.resized_max_voxsize,
            )
            if save_volumes is not None and output_dir is not None:
                save_volume(
                    image_tensor,
        ...
        if aug == 'bias_field':
            image_tensor = apply_bias_field(
                image_tensor,
                original_image.geom.voxsize,
                bias_field_probability=cfg.preprocessing.bias_field_probability,
                bias_field_max_magnitude=cfg.preprocessing.bias_field_max_magnitude,
                bias_field_smoothing_range=cfg.preprocessing.bias_field_smoothing_range,
            )
            if save_volumes is not None and output_dir is not None:
                save_volume(
                    image_tensor,
        ...
        if aug == 'biasFieldCorruption':
            image_tensor = apply_biasFieldCorruption(image_tensor)
            if save_volumes is not None and output_dir is not None:
                save_volume(
                    image_tensor,
                    original_image,
        ...
        if aug == 'sampleConditionalGMM':
            image_tensor = apply_sampleConditionalGMM(
                label_tensor, cfg.dataset.expected_classes
            )
            if save_volumes is not None and output_dir is not None:
                save_volume(
                    image_tensor,
        ...
        if aug == 'intensityAugmentation':
            image_tensor = apply_intensityAugmentation(image_tensor)
            if save_volumes is not None and output_dir is not None:
                save_volume(
                    image_tensor,
                    original_image,
