...
Usage: test_dataset.py 
       ...
       [--augment]
       [--deterministic]
       [--preprocessing_outdir <preprocessing_output_dir>]
...
def main():
    ...
    config, _, _, _, _, _, train_dataset, _ = Training.setup(config, preload_dataset=args.preload, create_loader=False, create_model=False)
...
def argument_parse():
    ...
    parser.add_argument("--preprocessing_outdir", type=str, help="Path to preprocessing output (with augmenting if requested)")
