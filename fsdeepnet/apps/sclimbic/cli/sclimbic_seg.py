...
class LimbicSegmenter:
    ...
    def __init__(self,
        ...
        self.device = device
        if (pt is not None and self.device is None):
    ...
    def segment(self, image):
        ...
        # reshape conformed image data to [C, H, W, D], add batch dimension -> [B, C, H, W, D]
        # ??? Q. without the copy(), got the following error when segmenting bert vsinus:
        # *** ValueError: At least one stride in the given numpy array is negative, and tensors with negative strides are not currently supported. (You can probably work around this by making a copy of your array  with array.copy().)

        conformed_tensor = pt.from_numpy(conformed.framed_data.copy()).movedim(-1, 0).unsqueeze(0).to(self.device)
        with pt.no_grad():
            (prediction, _) = self.model(conformed_tensor)
            #(prediction, _) = self.model(np.moveaxis(conformed.framed_data[np.newaxis], -1, 1).to(self.device))

        # move prediction tensor to cpu, reshape it to [B, H, W, D, C], remove batch dimension -> [H, W, D, C]
        prediction = prediction.cpu().movedim(1, -1).squeeze()
